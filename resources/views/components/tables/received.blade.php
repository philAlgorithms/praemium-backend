    ...
    <div class="card">
		...
		@foreach ($transfers as $i=>$transfer)              
			...
			<td class="text-xs font-weight-bold w-50">
				...
				<span class="text-truncate">{{ $transfer->sender->name }}</span>	
                      	    </div>
			</td>	
			<td class="text-xs text-start font-weight-bold">
                      	    <span class="my-2 text-x">{{ $transfer->description }}</span>
			</td>
		
			
<td class="text-xs text-center font-weight-bold">
                      	    <a href="transfer-received-{{ $transfer->uuid }}" class="my-2 text-x">More details&nbsp;&nbsp;</i></a>
			</td>
		    </tr>
		@endforeach
		</tbody>
